...
class Z3ModelExtractorTest extends ShouldMatchersForJUnit {
  ...
  def testCanExtractSimpleModel() {
    ...
    Z3ModelExtractor.extractAssignments(model)("name1").assignments.toString() should be("[cond1 (Bool) = false, q1 (Bool) = false, q3 (Bool) = false, q2 (Bool) = false, always_true_name1 (Bool) = false]")
  }
  ...
  def testCanExtractBrokenModel() {
    ...
    Z3ModelExtractor.extractAssignments(model)("analysis1").assignments.toString() should be("[cond1 (Bool) = false, always_true_analysis1 (Bool) = false, cond2 (Bool) = false]")
  }
  ...
  def testCanExtractModelFromExtendedSynthesis() {
    ...
    println(Z3ModelExtractor.extractAssignments(model)("name1"))
  }
}
