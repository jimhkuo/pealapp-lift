...
@init {combined = new HashMap<String, Object>(); combined.putAll(pols); combined.putAll(pSets); }
	:id0=IDENT '=' id1=IDENT {catchError(combined, "Policy $ is not declared but is used on line \"" + $id0.text + " = " + $id1.text + "\"", $id1.text); PolicySet p = new BasicPolicySet(pols.get($id1.text), $id0.text); pSets.put($id0.text, p);}
	|id0=IDENT '=' 'max' '(' id1=IDENT ',' id2=IDENT ')' {catchError(combined, "$ has not been declared as a policy or a policy set, but is used on line \"" + $id0.text + " = max(" + $id1.text + ", " +$id2.text + ")\"", $id1.text, $id2.text); PolicySet p = new MaxPolicySet(PolicyResolver.getFromOr(pols, pSets, $id1.text), PolicyResolver.getFromOr(pols, pSets, $id2.text), $id0.text); pSets.put($id0.text, p);}
	|id0=IDENT '=' 'min' '(' id1=IDENT ',' id2=IDENT ')' {catchError(combined, "$ has not been declared as a policy or a policy set, but is used on line \"" + $id0.text + " = min(" + $id1.text + ", " +$id2.text + ")\"", $id1.text, $id2.text);PolicySet p = new MinPolicySet(PolicyResolver.getFromOr(pols, pSets, $id1.text), PolicyResolver.getFromOr(pols, pSets, $id2.text), $id0.text); pSets.put($id0.text, p);}
	|id0=IDENT '=' '+' '(' id1=IDENT ',' id2=IDENT ')' {catchError(combined, "$ has not been declared as a policy or a policy set, but is used on line \"" + $id0.text + " = +(" + $id1.text + ", " +$id2.text + ")\"", $id1.text, $id2.text);PolicySet p = new PlusPolicySet(PolicyResolver.getFromOr(pols, pSets, $id1.text), PolicyResolver.getFromOr(pols, pSets, $id2.text), $id0.text); pSets.put($id0.text, p);}
	|id0=IDENT '=' '*' '(' id1=IDENT ',' id2=IDENT ')' {catchError(combined, "$ has not been declared as a policy or a policy set, but is used on line \"" + $id0.text + " = *(" + $id1.text + ", " +$id2.text + ")\"", $id1.text, $id2.text);PolicySet p = new MulPolicySet(PolicyResolver.getFromOr(pols, pSets, $id1.text), PolicyResolver.getFromOr(pols, pSets, $id2.text), $id0.text); pSets.put($id0.text, p);}
	;
