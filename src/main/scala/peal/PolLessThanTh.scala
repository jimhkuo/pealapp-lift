...
class PolLessThanTh(pol: Pol, th: Double) extends pSet {
  ...
  def synthesis(z3 : Z3Context) = pol.defaultScore match {
    case s if s <= th => "(or " + new DefaultLessThanTh(pol, th).synthesis(z3) + " " + new NonDefaultPolLessThanTh(pol, th).synthesis(z3) + ")"
    case _ => "(and " + new ThLessThanDefault(pol, th).synthesis(z3) + " " + new NonDefaultPolLessThanTh(pol, th).synthesis(z3) + ")"
  }
