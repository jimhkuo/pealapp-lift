...
class ExplicitSynthesiserTest extends ShouldMatchersForJUnit with Z3ModelMatcher {
  ...
  def testCanDealWithDefaultInput() {
    ...
    val output = new ExplicitSynthesiser(input).generate(doVacuityCheck = true)
  ...
  def testScalaFuture() {
    ...
    val outputFuture = future {
      new ExplicitSynthesiser(input).generate(doVacuityCheck = true)
    }
