...
class PealProgramParserTest extends ShouldMatchersForJUnit with Z3ModelMatcher {
  ...
  def testUndeclaredThrowExceptionImpliesAnalysis() {
    val ex = intercept[RuntimeException] {
      val input = "POLICIES\nb1 = + ((q0 0.1)(q1 0.1)(q2 0.1)(q3 0.1)(q4 0.1)) default 0\n" +
        ...
        "cond1 = q0\n" +
        "ANALYSES\n" +
        "analysis1 = implies? cond cond1"
      val pealProgramParser = ParserHelper.getPealParser(input)
      pealProgramParser.program()
    }
    ex.getMessage should be("Condition cond is not declared but is used on line \"analysis1 = implies? cond cond1\"")
  }
  ...
  def testUndeclaredThrowExceptionImpliesAnalysis1() {
    val ex = intercept[RuntimeException] {
      val input = "POLICIES\nb1 = + ((q0 0.1)(q1 0.1)(q2 0.1)(q3 0.1)(q4 0.1)) default 0\n" +
        ...
        "cond1 = q0\n" +
        "ANALYSES\n" +
        "analysis1 = implies? cond1 cond"
      val pealProgramParser = ParserHelper.getPealParser(input)
      pealProgramParser.program()
    }
    ex.getMessage should be("Condition cond is not declared but is used on line \"analysis1 = implies? cond1 cond\"")
  }
  ...
  def testCanIgnoreComments() {
    ...
    val allRules = pealProgramParser.pols.values().flatMap(pol => pol.rules).toSeq.sortWith((r1, r2) => r1.toString < r2.toString)
