...
class EqualsIgnoreWhiteSpaceWord(expectedInput: String) extends Matcher[Any] {
  ...
  private def compareModel(actual: Array[String], expected: Array[String]) : Boolean = {
    actual.size match {
      case s if s == expected.size => actual.forall(expected.contains)
      case _ => false
    }
  }
}
