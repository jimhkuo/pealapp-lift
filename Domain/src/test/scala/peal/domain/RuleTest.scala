...
class RuleTest extends ShouldMatchersForJUnit {
  ...
  def testCanOutputZ3VariableString() {
    new Rule(new Predicate("q1"), Right(VariableFormula(Multiplier(7, "x")))).toString should be ("(q1 (* 7 x))")
  }

  @Test
  def testCanReturnCorrectScoreString() {
    new Rule(new Predicate("q1"), Right(VariableFormula(Multiplier(7, "x")))).scoreString should be ("(* 7 x)")
