...
object ActToPeal {
  ...
  private def executeNext(act: Act): String = act match {
    ...
    case OrLeaf(name, lhs, rhs@_*) =>
      "prob_or_" + name + "= +((True 1.0) (True -1.0*prob_or_" + name + "_aux_score)) default 0.0\n" +
        "prob_or_" + name + "_aux = *(" + orAuxRules[Act](name, Seq(lhs) ++ rhs: _*).mkString + ") default 0.0\n" +
        orAux[Act](name, Seq(lhs) ++ rhs: _*).mkString +
        executeNext(lhs) +
        rhs.map(executeNext).mkString
  ...
  private def executeNext(dmAct: DmAct): String = dmAct match {
    ...
    case OrDmAct(name, lhs, rhs@_*) =>
      "prob_or_" + name + "= +((True 1.0) (True -1.0*prob_or_" + name + "_aux_score)) default 0.0\n" +
        "prob_or_" + name + "_aux = *(" + orAuxRules[DmAct](name, Seq(lhs) ++ rhs: _*).mkString + ") default 0.0\n" +
        orAux[DmAct](name, Seq(lhs) ++ rhs: _*).mkString +
        executeNext(lhs) +
        rhs.map(executeNext).mkString
  }
