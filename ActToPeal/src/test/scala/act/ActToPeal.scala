...
object ActToPeal {
  ...
  private def orAux(name: String, acts: Act*) = for ((act, i) <- acts.zipWithIndex) yield ("prob_or_" + name + "_aux" + i + " = +((True 1.0) (True -1.0*" + policyName(act) + ")) default 0.0\n")
  ...
  private def policyName(act: Act): String = act match {
    case AndLeaf(name, _, _) => "prob_and_" + name + "_score"
    case OrLeaf(name, _, _) => "prob_or_" + name + "_score"
